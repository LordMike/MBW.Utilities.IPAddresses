...
namespace MBW.Utilities.IPAddresses
{
    public partial struct IpAddressRangeV6
    {
        ...
        public static bool TryParse(ReadOnlySpan<char> value, out IpAddressRangeV6 result)
        {
            ...
            {
                (TokenType type, ushort value) slashTkn = Tokenizer.ReadTokenReverse(value.Slice(0, value.Length - read), false, out int read2);
                if (slashTkn.type == TokenType.Slash && tkn.value <= 128)
                {
                    cidr = (byte)tkn.value;
                    value = value.Slice(0, value.Length - read - read2);
                }
            ...
            if (doReverse)
            {
                ...
                for (byte i = 0; i < toRead; i++)
                {
                    ...
                    value = value.Slice(0, value.Length - read);
            ...
            if (doReverse)
            {
                ...
                while (value.Length > 0)
                {
                    ...
                    if (lastIdx == 0 && value.Length >= 7)
                    {
                        ...
                        if (!IpAddressRangeV4.TryParse(value.Slice(1), out IpAddressRangeV4 ipv4))
                            return default;
